...
func _prepare_scene() -> void:
	...
	if _out_dir.contains("collision"):
		# CollisionLayer nodes live inside the instanced world chunks now.
		var world_map := _level.get_node_or_null("WorldMap")
		if world_map:
			for chunk in world_map.get_children():
				var overlay := chunk.get_node_or_null("CollisionLayer") as TileMapLayer
				if overlay:
